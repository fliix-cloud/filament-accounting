...
| ACC-AUD-01 | Make critical actions sequential, tamper-evident, attributable, and chronologically verifiable per legal entity. | Engineering; `AuditLogger`, `AuditEventHasher`, `AuditChainVerifier`, audit schema. | `AuditLedgerTest`; `filament-accounting:verify` output; chain head and events. | Partial | A chain head outside the application database is still required to withstand a privileged database attacker. |
...
| ACC-VER-01 | Detect integrity, sequence, relation, amount, tenant, retention, and export faults with human and machine-readable output. | Verification owner; `filament-accounting:verify`. | Intentional manipulation suite and CI artifacts. | Partial | The command now verifies ledger balance and audit chains; remaining evidence domains and JSON output are open. |
