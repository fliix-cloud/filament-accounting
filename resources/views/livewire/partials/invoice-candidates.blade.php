    ...
    @else
        <table class="fac-table">
            <thead>
                <tr>
                    <th>
                        {{ $candidateType === 'sales_invoice' ? __('filament-accounting::fields.number') : __('filament-accounting::fields.internal_number') }}
                    </th>
                    <th>
                        {{ $candidateType === 'purchase_invoice' ? __('filament-accounting::fields.supplier') : __('filament-accounting::fields.customer') }}
                    </th>
            ...
            <tbody>
                @foreach ($candidates as $candidate)
                    <tr
                        @class(['fac-selected-row' => $this->selectedOpenItemId === $candidate['id']])
                        wire:key="reconciliation-candidate-{{ $candidateType }}-{{ $candidate['id'] }}"
                    >
                        <td>
                        ...
                        <td class="fac-number fac-open-amount">
                            {{ \FilamentAccounting\Support\MoneyFormatter::format($candidate['remaining_minor'], $candidate['currency']) }}
                        </td>
                        <td>
                        ...
                        <td>
                            @if ($candidate['score'] > 0)
                                ...
                                <details class="fac-row-details">
                                    <summary>
                                        {{ __('filament-accounting::fields.suggestion_rank', ['rank' => $loop->iteration]) }} · {{ __('filament-accounting::fields.show_details') }}
                                    </summary>
                                    <span class="fac-reasons">
                                        @foreach ($candidate['reasons'] as $reason) {{ __('filament-accounting::fields.match_reasons.'.$reason) }}@if (! $loop->last) · @endif @endforeach
                                        @if ($candidate['ambiguous']) ·{{ __('filament-accounting::fields.ambiguous_match') }} @endif
                                    </span>
                                </details>
                            @else
                        ...
                        <td>
                            ...
                            >{{ $this->selectedOpenItemId === $candidate['id'] ? __('filament-accounting::fields.selected') : __('filament-accounting::actions.select') }}</x-filament::button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
