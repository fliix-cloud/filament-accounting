...
<body>
    <footer>
        <table>
            <tr>
                <td>
                    @if (filled($seller['invoice_bank_name'] ?? null))
                        {{ __('filament-accounting::invoice.bank') }}: {{ $seller['invoice_bank_name'] }}<br
                         />
                    @endif
                    @if (filled($seller['invoice_iban'] ?? null))
                        IBAN: {{ $seller['invoice_iban'] }}<br
                         />
                    @endif
                ...
                <td>
                    @if (filled($seller['tax_number'] ?? null))
                        {{ __('filament-accounting::fields.tax_number') }}: {{ $seller['tax_number'] }}<br
                         />
                    @endif
    ...
    <table class="masthead">
        <tr>
            ...
            <td style="width: 68mm; text-align: right">
                @if (filled($seller['invoice_logo_data'] ?? null))
                    <img class="logo" src="{{ $seller['invoice_logo_data'] }}" alt="" />
                @endif
            </td>
        </tr>
    </table>
        ...
        <tr>
            <td class="recipient">
                <div class="sender-line">
                    {{ $seller['trading_name'] ?? $seller['legal_name'] ?? '' }} - {{ $seller['address_line1'] ?? '' }} - {{ $seller['postal_code'] ?? '' }} {{ $seller['city'] ?? '' }}
                </div>
                {{ $buyer['legal_name'] ?? '' }}<br />
                {{ $address['line1'] ?? '' }}<br />
                ...
                @if (($address['country_code'] ?? $buyer['country_code'] ?? '') !== ($seller['country_code'] ?? ''))
                    <br
                    />{{ $address['country_code'] ?? $buyer['country_code'] ?? '' }}
                @endif
            </td>
            <td class="contact">
                <strong>{{ __('filament-accounting::invoice.contact') }}:</strong><br />
                {{ $seller['legal_name'] ?? '' }}<br />{{ $seller['address_line1'] ?? '' }}<br />
                @if (filled($seller['address_line2'] ?? null))
                    {{ $seller['address_line2'] }}<br
                     />
                @endif
                {{ $seller['postal_code'] ?? '' }} {{ $seller['city'] ?? '' }}<br /><br />
                @if (filled($seller['email'] ?? null))
                    {{ __('filament-accounting::fields.email') }}: {{ $seller['email'] }}<br
                     />
                @endif
                @if (filled($seller['phone'] ?? null))
                    {{ __('filament-accounting::fields.phone') }}: {{ $seller['phone'] }}<br
                     />
                @endif
                <table class="metadata">
                    <tr>
                        <td>{{ __('filament-accounting::invoice.date') }}:</td>
                        <td>{{ $date($snapshot['issue_date'] ?? null) }}</td>
                    </tr>
                    <tr>
                        <td>{{ __('filament-accounting::invoice.number') }}:</td>
                        <td>{{ $snapshot['number'] ?? '' }}</td>
                    </tr>
                    @if (filled($buyer['external_reference'] ?? null))
                        <tr>
                            <td>{{ __('filament-accounting::invoice.customer_number') }}:</td>
                            <td>{{ $buyer['external_reference'] }}</td>
                        </tr>
                    @endif
                    @if (filled($seller['invoice_contact_name'] ?? null))
                        <tr>
                            <td>{{ __('filament-accounting::invoice.contact_person') }}:</td>
                            <td>{{ $seller['invoice_contact_name'] }}</td>
                        </tr>
                    @endif
                </table>
            </td>
    ...
    @if (isset($snapshot['invoice_version']))
        <div class="revision">
            {{ __('filament-accounting::fields.invoice_version') }} {{ $snapshot['invoice_version'] }} — {{ $snapshot['correction_reason'] ?? '' }}
        </div>
    @endif
    <div class="intro">
        <p>{{ __('filament-accounting::invoice.salutation') }}</p>
        <p>{{ __('filament-accounting::invoice.intro') }}</p>
    </div>
    <table class="items">
        <thead>
            <tr>
    ...
    <table class="totals">
        <tr>
            <td>{{ __('filament-accounting::invoice.subtotal') }}</td>
            <td class="right">{{ $money((int) ($snapshot['net_minor'] ?? 0)) }}</td>
        </tr>
        @foreach ($taxes as $tax)
            <tr>
                <td>
                    {{ __('filament-accounting::invoice.tax', ['rate' => str_replace('.', ',', (string) ($tax['rate_bp'] / 100)), 'net' => $money($tax['net_minor'])]) }}
                    @if (filled($tax['reason'])) —{{ $tax['reason'] }}@endif
                </td>
                <td class="right">{{ $money($tax['tax_minor']) }}</td>
            </tr>
        @endforeach
        <tr class="grand">
            <td>{{ __('filament-accounting::invoice.total') }}</td>
            <td class="right">{{ $money((int) ($snapshot['gross_minor'] ?? 0)) }}</td>
        </tr>
    </table>
    <div class="payment">
        ...
        @if (filled($snapshot['payment']['mandate_reference'] ?? null))
            {{ __('filament-accounting::invoice.mandate') }}: {{ $snapshot['payment']['mandate_reference'] }}<br
             />
        @endif
        @if (filled($snapshot['due_date'] ?? null))
            {{ __('filament-accounting::fields.due_date') }}: {{ $date($snapshot['due_date']) }}<br
             />
        @endif
