<div
    @class(['space-y-4', 'filament-accounting-sca-modal'])
    @if ($shouldPoll ?? false) wire:poll.3s="pollScaChallenge" @endif
>
    @if ($session)
        <div>
            ...
            @if ($session->expires_at)
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    {{ __('filament-accounting::banking/fints/sca.expires') }}: {{ $session->expires_at->timezone(config('app.timezone'))->format('H:i:s') }}
                </div>
            @endif
        </div>
        ...
        @if (($session->state->value ?? null) === 'needs_decoupled')
            <p class="text-sm text-gray-600 dark:text-gray-300">
                {{ __('filament-accounting::banking/fints/sca.poll_hint') }}
            </p>
        @endif

        @if (filled($vopText ?? null))
            <div class="bg-warning-50 text-warning-800 dark:bg-warning-400/10 dark:text-warning-400 rounded-lg p-4 text-sm">
